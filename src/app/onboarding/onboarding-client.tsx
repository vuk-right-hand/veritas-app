...
export default function Onboarding({ oauthUser }: OnboardingProps) {
    ...
    // We keep a state for the oauth user, populated initially from server props
    // but falling back to client-side auth state to be absolutely certain we don't miss it.
    const [clientOauthUser, setClientOauthUser] = useState<{ id: string; email: string; name: string } | null>(oauthUser);
    ...
    const handleStruggleSelect = (selected: string) => {
        if (selected === "Other...") {
            ...
            handleNext(undefined, selected);
        }
    };

    const handleNext = async (overrideGoal?: string, overrideStruggle?: string) => {
        setError('');

        if (step === 2 && clientOauthUser) {
            // User is already authenticated via OAuth -> skip steps 3 and 4
            setLoading(true);
            const currentGoal = overrideGoal || (formData.goal === "Other" ? customGoal : formData.goal);
            const currentStruggle = overrideStruggle || (formData.struggle === "Other" ? customStruggle : formData.struggle);

            try {
                const result = await saveMissionForOAuthUser(
                    clientOauthUser.id,
                    clientOauthUser.email,
                    clientOauthUser.name,
                    currentGoal,
                    currentStruggle
                );
