-- Add p_published_after (5th arg) to get_personalized_feed for temporal filtering.
-- NULL = evergreen (no cutoff). Non-null = videos.published_at >= cutoff.
-- Curations are injected client-side and are exempt from this filter by design.
--
-- Drop the existing 4-arg shape explicitly — Postgres treats different arg
-- counts as distinct functions, so CREATE OR REPLACE alone won't replace it.
...
GRANT EXECUTE ON FUNCTION public.get_personalized_feed(TEXT, INT, INT, TEXT[], TIMESTAMPTZ)
